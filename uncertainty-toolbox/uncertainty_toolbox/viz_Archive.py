...
def plot_parity(
    ...
    ax.legend(
        [h1[0]], ['y = x'], loc=leg_loc,
    )

    # Determine lims

    if ylims is None:
        lower_upper = [y_pred, y_true]
        lims_ext = [
            int(np.floor(np.min(lower_upper))),
            int(np.ceil(np.max(lower_upper))),
        ]
    else:
        lims_ext = ylims
...
def plot_xy(
    ...
    h2 = ax.plot(x, y_pred, '--', c='#ff7f0e', linewidth=2)
    h3 = ax.fill_between(
...
def plot_intervals_ordered(
    ...
    h1 = ax.plot(xs, y_pred, '.', mec='#1f77b4', mfc='#1f77b4')
...
def plot_residuals_vs_stds(
    ...
    h1 = ax.plot(y_std_scaled, np.abs(residuals), '.', c='#1f77b4')
