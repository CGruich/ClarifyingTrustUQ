...
def make_plots(pred_mean, pred_std, target, subsetSeed, lineplot_array=None, lineplot_legLabels=None, lineplot_xLabels=None, showTitleChoice=True, showLegendChoice=True, plot_save_str="row"):
    ...
    uct.viz_MC_Dropout.update_rc("xtick.labelsize", 14)
    # Change ytick label size
    # Set font size for yaxis tick labels
    uct.viz_MC_Dropout.update_rc("ytick.labelsize", 14)
    # Turn off Latex mode
    ...
    axs[0][0] = uct.viz_MC_Dropout.plot_intervals_ordered(
        pred_mean, pred_std, target, n_subset=subsetCountOrderedIntervals, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[
            0][0]
    )
    ...
    # Make sharpness plot
    axs[0][2] = uct.viz_MC_Dropout.plot_sharpness(
        pred_mean, pred_std, target, subsetCountSharpness, bins=binCount, seed=subsetSeed, showTitle=showTitleChoice, ax=axs[0][2])

    # Make residuals vs. uncertainties plot
    axs[1][0] = uct.viz_MC_Dropout.plot_residuals_vs_stds(
        pred_mean, pred_std, target, subsetCountResidualUncertainty, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[1][0])

    # Plot adversarial group calibration curve.
    axs[3][0] = uct.viz_MC_Dropout.plot_adversarial_group_calibration(
        pred_mean, pred_std, target, subsetCountAdversarialCalibration, seed=subsetSeed, showTitle=showTitleChoice, ax=axs[3][0])

    # Make a parity plot, this shows model accuracy on a sample-by-sample basis
    axs[2][0] = uct.viz_MC_Dropout.plot_parity(pred_mean, pred_std, target, subsetCountParity, leg_loc=4,
                                               seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[2][0])

    gridSizeCount = 100
    # Make a parity plot, but with hexagonal binning. This shows model accuracy on a sample-by-sample basis.
    axs[2][1] = uct.viz_MC_Dropout.plot_parity_hexagonal(pred_mean, pred_std, target, gridSize=gridSizeCount, subFig=fig,
    ...
    axs[2][2] = uct.viz_MC_Dropout.plot_parity_hexagonal(pred_mean, pred_std, target, gridSize=gridSizeCount, bins="log", subFig=fig,
                                                         n_subset=None, leg_loc=4, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[2][2])

    # Residual Error vs Uncertainty but with hexagonal binning
    axs[1][1] = uct.viz_MC_Dropout.plot_residuals_vs_stds_hexagonal(pred_mean, pred_std, target, gridSize=gridSizeCount, bins="log",
                                                                    subFig=fig, n_subset=None, leg_loc=4, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[1][1])

    # Violin/Box plot
    axs[1][2] = uct.viz_MC_Dropout.plot_violin_box_plot(pred_std, ylims=None, leg_loc=4, seed=subsetSeed, kdePoints=1000, width=0.5, showTitle=showTitleChoice,
                                                        showLegend=showLegendChoice, showMeans=False, showExtrema=False, showMedians=False, showVertical=True, ax=axs[1][2])

    if type(lineplot_array) != type(None):
        # Accuracy lineplot to assess MC dropout accuracy at different dropout rates
        axs[3][1] = uct.viz_MC_Dropout.plot_accuracy_lineplot(
...
lineplotDataArray = np.asarray([(0, 0.650565663721069), (5, 0.656844056111952), (
    10, 0.6692073030924), (20, 0.742656429820688), (30, 0.890508384605024)], dtype="float32")
