...
def make_plots(pred_mean, pred_std, target, subsetSeed, lineplot_array = None, lineplot_legLabels = None, lineplot_xLabels = None, showTitleChoice = True, showLegendChoice = True, plot_save_str="row"):    
    ...
    uct.viz_Ensemble.update_rc("xtick.labelsize", 14)  # Set font size for xaxis tick labels
    # Change ytick label size
    uct.viz_Ensemble.update_rc("ytick.labelsize", 14)  # Set font size for yaxis tick labels
    # Turn off Latex mode
    ...
    axs[0][1] = uct.viz_Ensemble.plot_calibration(pred_mean, pred_std, target, seed=subsetSeed, showTitle=showTitleChoice, showLegend=False, ax=axs[0][1])

    # Make sharpness plot
    axs[0][2] = uct.viz_Ensemble.plot_sharpness(pred_mean, pred_std, target, subsetCountSharpness, bins=binCount, seed=subsetSeed, showTitle=showTitleChoice, ax=axs[0][2])

    # Make residuals vs. uncertainties plot
    axs[1][0] = uct.viz_Ensemble.plot_residuals_vs_stds(pred_mean, pred_std, target, subsetCountResidualUncertainty, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[1][0])

    # Plot adversarial group calibration curve.
    axs[3][0] = uct.viz_Ensemble.plot_adversarial_group_calibration(pred_mean, pred_std, target, subsetCountAdversarialCalibration, seed=subsetSeed, showTitle=showTitleChoice, ax=axs[3][0])

    # Make a parity plot, this shows model accuracy on a sample-by-sample basis
    axs[2][0] = uct.viz_Ensemble.plot_parity(pred_mean, pred_std, target, subsetCountParity, leg_loc=4, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[2][0])

    gridSizeCount = 100
    # Make a parity plot, but with hexagonal binning. This shows model accuracy on a sample-by-sample basis.
    axs[2][1] = uct.viz_Ensemble.plot_parity_hexagonal(pred_mean, pred_std, target, gridSize=gridSizeCount, subFig=fig, n_subset = None, leg_loc=4, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[2][1])

    # Hexagonal plot, but with log color scale and bins
    axs[2][2] = uct.viz_Ensemble.plot_parity_hexagonal(pred_mean, pred_std, target, gridSize=gridSizeCount, bins="log", subFig=fig, n_subset = None, leg_loc=4, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[2][2])

    axs[1][1] = uct.viz_Ensemble.plot_residuals_vs_stds_hexagonal(pred_mean, pred_std, target, gridSize=gridSizeCount, bins="log", subFig=fig, n_subset = None, leg_loc=4, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[1][1])

    # Violin + Box plot
    axs[1][2] = uct.viz_Ensemble.plot_violin_box_plot(pred_std, ylims=None, leg_loc=4, seed=subsetSeed, kdePoints=1000, width=0.5, whiskers=(1.0, 99.0), showTitle=showTitleChoice, showLegend=showLegendChoice, showMeans=False, showExtrema=False, showMedians=False, showVertical=True, ax=axs[1][2])

    if type(lineplot_array) != type(None):
        # Accuracy lineplot to assess MC dropout accuracy at different dropout rates
        axs[3][1] = uct.viz_MC_Ensemble.plot_accuracy_lineplot(lineplot_array, leg_loc = 2, leg_labels = lineplot_legLabels, xLabels = lineplot_xLabels, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[3][1])
...
make_plots(predictionNP, uncertaintyNP, targetNP, rngVisualSeed, lineplot_array = None, lineplot_legLabels = None, lineplot_xLabels = None,
