...
def make_plots(pred_mean, pred_std, target, lamb, subsetSeed, lineplot_array=None, lineplot_legLabels=None, lineplot_xLabels=None, showTitleChoice=True, showLegendChoice=True, plot_save_str="row"):
    ...
    fig.suptitle("\nUncertainty Quantification Plots\n(Evidential Regression, " +
                 lambdaSymbolStr + " = " + str(lamb) + ")", fontsize=24)

    print(axs)

    # Make ordered intervals plot
    axs[0][0] = uct.viz_Evidential.plot_intervals_ordered(
        pred_mean, pred_std, target, lamb, n_subset=subsetCountOrderedIntervals, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[
            0][0]
    )

    # Make calibration plot
    axs[0][1] = uct.viz_Evidential.plot_calibration(
        pred_mean, pred_std, target, lamb, seed=subsetSeed, showTitle=showTitleChoice, ax=axs[0][1])

    # Make sharpness plot
    axs[0][2] = uct.viz_Evidential.plot_sharpness(
        pred_mean, pred_std, target, lamb, subsetCountSharpness, bins=binCount, seed=subsetSeed, showTitle=showTitleChoice, ax=axs[0][2])

    # Make residuals vs. uncertainties plot
    axs[1][0] = uct.viz_Evidential.plot_residuals_vs_stds(
        pred_mean, pred_std, target, lamb, subsetCountResidualUncertainty, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[1][0])

    # Plot adversarial group calibration curve.
    axs[3][0] = uct.viz_Evidential.plot_adversarial_group_calibration(
        pred_mean, pred_std, target, lamb, subsetCountAdversarialCalibration, seed=subsetSeed, showTitle=showTitleChoice, ax=axs[3][0])

    # Make a parity plot, this shows model accuracy on a sample-by-sample basis
    axs[2][0] = uct.viz_Evidential.plot_parity(
        pred_mean, pred_std, target, lamb, subsetCountParity, leg_loc=4, seed=subsetSeed, showLegend=showLegendChoice, ax=axs[2][0])

    gridSizeCount = 100
    # Make a parity plot, but with hexagonal binning. This shows model accuracy on a sample-by-sample basis.
    axs[2][1] = uct.viz_Evidential.plot_parity_hexagonal(pred_mean, pred_std, target, lamb, gridSize=gridSizeCount, subFig=fig,
                                                         n_subset=None, leg_loc=4, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[2][1])

    # Hexagonal plot, but with log color scale and bins
    axs[2][2] = uct.viz_Evidential.plot_parity_hexagonal(pred_mean, pred_std, target, lamb, gridSize=gridSizeCount, bins="log",
                                                         subFig=fig, n_subset=None, leg_loc=4, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[2][2])

    axs[1][1] = uct.viz_Evidential.plot_residuals_vs_stds_hexagonal(pred_mean, pred_std, target, lamb, gridSize=gridSizeCount, bins="log",
                                                                    subFig=fig, n_subset=None, leg_loc=4, seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[1][1])

    # Violin + Box plot
    axs[1][2] = uct.viz_Evidential.plot_violin_box_plot(pred_std, lamb, ylims=None, leg_loc=4, seed=subsetSeed, kdePoints=1000, width=0.5,
                                                        showTitle=showTitleChoice, showLegend=showLegendChoice, showMeans=False, showExtrema=False, showMedians=False, showVertical=True, ax=axs[1][2])

    if type(lineplot_array) != type(None):
        # Accuracy lineplot to assess MC dropout accuracy at different dropout rates
        axs[3][1] = uct.viz_Evidential.plot_accuracy_lineplot(
            lineplot_array, leg_loc=2, leg_labels=lineplot_legLabels, xLabels=lineplot_xLabels, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs[3][1])
...
lineplotDataArray = np.asarray([(0.0, 0.633265837888014), (0.05, 0.629793262621284), (
    0.1, 0.631996560722256), (0.15, 0.632487070559222), (0.2, 0.633375750628576)], dtype="float32")
...
aleatoricMace = uct.mean_absolute_calibration_error(
