...
def make_plots(
    ...
    axs[0], countsEnsemble = uct.viz_Ensemble.plot_parity_hexagonal(
        pred_mean[0],
        pred_std[0],
        target,
        ...
        showColorBar=False,
        getCounts=True,
        statLabels=accuracyStats[0],
        ax=axs[0],
    )
    axs[0].set(xlabel=None)
    if showTitleChoice == True:
        axs[0].set(title='5-fold Ensemble')
    axs[1], countsDropout = uct.viz_MC_Dropout.plot_parity_hexagonal(
        pred_mean[1],
        pred_std[1],
        target,
        gridSize=gridSizeCount,
        ...
        showColorBar=False,
        getCounts=True,
        statLabels=accuracyStats[1],
        ax=axs[1],
    )
    axs[1].set(ylabel=None)
    if showTitleChoice == True:
        axs[1].set(title='MC Dropout')
    axs[2], countsEvidential = uct.viz_Evidential.plot_parity_hexagonal(
        pred_mean[2],
        pred_std[2],
        target,
        lamb,
    ...
    axs[0], countsEnsemble = uct.viz_Ensemble.plot_parity_hexagonal(
        ...
        subFig=fig,
        n_subset=None,
        ylims=commonPlotLims[0],
        xlims=commonPlotLims[1],
        leg_loc=4,
        colorRange=colorBarLims,
        seed=subsetSeed,
        showTitle=showTitleChoice,
        showLegend=showLegendChoice,
        showColorBar=False,
    ...
    scalarMap = plt.cm.ScalarMappable(
        cmap='Blues', norm=mpl.colors.LogNorm(vmin=colorBarLims[0], vmax=colorBarLims[1])
    )
    ...
    colorBarAx = plt.axes(
        [
            rightMostPlotCorners[1][0] + padding,
            rightMostPlotCorners[0][1],
            0.0125,
            rightMostPlotPosition.height,
        ]
    )
...
savePlotNameTitle = 'UQ_Technique_Compare_Accuracy_Hexbin_Log_Title_SameAxes_Seed' + str(
    rngVisualSeed
)
savePlotNameLeg = 'UQ_Technique_Compare_Accuracy_Hexbin_Log_Leg_SameAxes_Seed' + str(
    rngVisualSeed
)
