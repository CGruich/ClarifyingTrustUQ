...
def make_plots(pred_mean, pred_std, target, lamb, subsetSeed, lineplot_array = None, lineplot_legLabels = None, lineplot_xLabels = None, accuracyStats = None, showTitleChoice = True, showLegendChoice = True, plot_save_str="row"):
    ...
    plotLimBuffer = 0.1
    commonPlotLims = ((commonTickLims[0][0] - plotLimBuffer, commonTickLims[0][1] + plotLimBuffer), (commonTickLims[1][0] - plotLimBuffer, commonTickLims[1][1] + plotLimBuffer))
    ...
    axs[0].tick_params(axis="both", which="major", direction="out", color="black", length=4.0, width=2.0)
    if showTitleChoice == True:
        axs[0].set(title = "5-fold Ensemble")
    ...
    axs[1].tick_params(axis="both", which="major", direction="out", color="black", length=4.0, width=2.0)
    if showTitleChoice == True:
        axs[1].set(title = "MC Dropout")
    ...
    axs[2].tick_params(axis="both", which="major", direction="out", color="black", length=4.0, width=2.0)
    if showTitleChoice == True:
        axs[2].set(title = "Evidential Regression")

    plt.tight_layout()
    #colorBarDivider = make_axes_locatable(axs[2])
    scalarMap = plt.cm.ScalarMappable(cmap="Blues", norm=mpl.colors.LogNorm(vmin=colorBarLims[0], vmax=colorBarLims[1]))
    ...
    colorBarAx = plt.axes([rightMostPlotCorners[1][0] + padding, rightMostPlotCorners[0][1], 0.0125, rightMostPlotPosition.height])
...
savePlotNameTitle = "UQ_Technique_Compare_Accuracy_Hexbin_Log_Title_SameAxes_Seed" + str(rngVisualSeed)
savePlotNameLeg = "UQ_Technique_Compare_Accuracy_Hexbin_Log_Leg_SameAxes_Seed" + str(rngVisualSeed)
savePlotNameLegTitle = "UQ_Technique_Accuracy_Hexbin_Log_LegTitle_SameAxes_Seed" + str(rngVisualSeed)
