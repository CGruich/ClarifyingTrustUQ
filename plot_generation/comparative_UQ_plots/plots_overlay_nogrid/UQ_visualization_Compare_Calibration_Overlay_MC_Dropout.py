...
def make_plots(
    ...
    axs = uct.viz_MC_Dropout.plot_calibration(
        pred_mean,
        pred_std,
        target,
        leg_loc=2,
        leg_labels=sampleList,
        legendTitle='(Samples, Miscalibration Area)',
        seed=subsetSeed,
...
for ind in range(len(sampleList)):
    predictionFilePath = (
        os.path.join(predictionFilePathList[0], predictionFileNameList[0])
        + sampleList[ind]
        + '.csv'
    )
    uncertaintyFilePath = (
        os.path.join(uncertaintyFilePathList[0], uncertaintyFileNameList[0])
        + sampleList[ind]
        + '.csv'
    )
...
# What to name plot
savePlotNameTitle = 'UQ_Technique_Compare_Overlay_Calibration_MC_Dropout_Title_Seed' + str(
    rngVisualSeed
)
savePlotNameLeg = 'UQ_Technique_Compare_Overlay_Calibration_MC_Dropout_Leg_Seed' + str(
    rngVisualSeed
)
