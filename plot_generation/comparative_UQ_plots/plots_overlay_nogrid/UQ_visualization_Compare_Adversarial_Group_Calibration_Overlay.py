...
def make_plots(pred_mean, pred_std, target, lamb, subsetSeed, lineplot_array = None, lineplot_legLabels = None, lineplot_xLabels = None, showTitleChoice = True, showLegendChoice = True, plot_save_str="row"):
    ...
    axs = uct.viz_Ensemble.plot_adversarial_group_calibration(pred_mean, pred_std, target, leg_loc=4, leg_labels=["5-fold Ensemble", "MC Dropout", epistemicLabel], lineList=["dashed", "dashed", "dashed", "dashed", "dashed"], 
            cali_type = "miscal_area", seed=subsetSeed, showTitle=showTitleChoice, showLegend=showLegendChoice, ax=axs)
    axs.yaxis.set_ticks(np.arange(0.0, 0.6, 0.1))
    axs.xaxis.set_ticks(np.arange(0.0, 1.2, 0.2))
    axs.set_ylim(0.1, 0.51)
...
# Where to save plot
savePlotPath = "ADVERSARIAL_GROUP_PLOT_SAVE_FILEPATH_USER_SPECIFIED"
